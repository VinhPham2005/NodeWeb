...
async function dbLoad() {
  try {
    ...
    console.error("Unable connecting to MongoDB Atlas!", error);
    process.exit(1);
  }
  ...
  for (const user of userModels) {
    ...
    const generatedLoginName = user.last_name.toLowerCase();
    ...
    try {
      ...
      console.log(
        ...
        " (Login: " + generatedLoginName + ") with ID ",
        user.objectID
      );
    } catch (error) {
